...
import { HeartButton, AuthCheck } from '../components';
...
function PostCard({ post, admin = false }) {
  ...
  useEffect(() => {
    const grabUser = async () => {
      const user = await getUserWithUsername(post.username);
      setUserDoc(user);
    }
    grabUser();
  }, [post.username])
  
  const postRef = userDoc?.ref.collection('posts').doc(post.slug);
